...
export default function FundExtractionPage() {
  ...
  const handleFileUpload = useCallback(async (files: File | File[]) => {
    ...
    setExtractionLogs(prev => [...prev, `📤 Uploading ${fileArray.length} file(s)...`]);
    ...
    try {
      ...
      for (const file of fileArray) {
        ...
        if (isCSV) {
          ...
          setExtractionLogs(prev => [...prev, `🔄 Creating session for ${file.name}...`]);
  ...
  const startExtraction = async (pathToProcess: string) => {
    ...
    if (isCSV) {
      ...
      setExtractionLogs(prev => [...prev, "📡 Connected to extraction stream..."]);
            ...
            if (line.startsWith('data: ')) {
              try {
                ...
                if (eventData.type === 'status_update') {
                  ...
                  setExtractionLogs(prev => [...prev, `🔄 ${agent}: ${stage} (${status})`]);
                } else if (eventData.type === 'data_processed') {
                  const portfolioItems = eventData.data?.portfolio_items || [];
                  setExtractionLogs(prev => [...prev, `✅ Found ${portfolioItems.length} funds`]);
                  ...
                  setExtractionLogs(prev => [...prev, `💬 ${eventData.data?.message}`]);
                }
              } catch (parseError) {
            ...
            if (line.startsWith('data: ')) {
              try {
                ...
                if (eventData.type === 'status') {
                  ...
                  setExtractionLogs(prev => [...prev, `🔄 ${stage} (${progress}%)`]);
                } else if (eventData.type === 'fund_extracted') {
                  const fund = eventData.data;
                  setFundData(prev => [...prev, fund]);
                  setExtractionLogs(prev => [...prev, `✅ Extracted: ${fund.name} (${fund.ticker})`]);
            ...
            if (line.startsWith("data: ")) {
              try {
                ...
                if (eventData.type === "connected") {
                  setExtractionLogs(prev => [...prev, "📡 Connected to processing stream"]);
                } else if (eventData.type === "chat_response") {
                  setExtractionLogs(prev => [...prev, `🤖 ${eventData.data.message}`]);
                } else if (eventData.type === "status_update") {
                  setExtractionStage(eventData.data.stage || eventData.data.status || "");
                  setExtractionLogs(prev => [...prev, `📊 ${eventData.data.status}: ${eventData.data.stage || 'processing'}`]);
                } else if (eventData.type === "data_processed") {
                  const portfolioData = eventData.data.portfolio_items || [];
                  setExtractionLogs(prev => [...prev, `✅ Processed ${portfolioData.length} portfolio items`]);
                  ...
                  setExtractionStatus("error");
                  setExtractionLogs(prev => [...prev, `❌ Error: ${eventData.data.error}`]);
                } else if (eventData.type === "results") {
    ...
    switch (event.type) {
      ...
      case "status":
        ...
        if (event.data.message) {
          setExtractionLogs(prev => [...prev, `📊 ${event.data.message}`]);
        }
        break;
      ...
      case "fund_discovered":
        setExtractionLogs(prev => [
          ...prev,
          `🔍 Found fund: ${event.data.fund_name} (${event.data.page_count} pages)`
        ]);
        break;
      ...
      case "results":
        ...
        setExtractionLogs(prev => [
          ...prev,
          `✅ Extraction completed! Found ${funds.length} funds`
        ]);
        break;
